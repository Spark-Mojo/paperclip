...
for p in shared db server; do d="$CLI_ROOT/node_modules/@paperclipai/$p"; mkdir -p "$d"; printf '{"name":"@paperclipai/%s","version":"2026.831.1","main":"./dist/index.js","exports":{".":{"types":"./dist/index.d.ts","import":"./dist/index.js"}}}\n' "$p" > "$d/package.json"; mkdir -p "$d/dist"; printf 'old\n' > "$d/dist/index.js"; done
...
MISMATCH_PREFIX="$SANDBOX/mismatch-root-prefix"; cp -a "$OVERLAY_PREFIX" "$MISMATCH_PREFIX"; node -e 'const fs=require("fs"),p=process.argv[1],x=JSON.parse(fs.readFileSync(p));x.name="@paperclipai/wrong";fs.writeFileSync(p,JSON.stringify(x))' "$MISMATCH_PREFIX/lib/node_modules/paperclipai/node_modules/@paperclipai/db/package.json"
capture mismatch_root_out mismatch_root_code node "$ENGINE_DIR/overlay-contract.mjs" "$MISMATCH_PREFIX" "$OVERLAY_BUILD" 0123456789012345678901234567890123456789 "$MISMATCH_PREFIX/.paperclip-engine-overlay.json"
assert_eq "mismatched package identity fails closed" "1" "$mismatch_root_code"
ESCAPE_PREFIX="$SANDBOX/escape-root-prefix"; ESCAPE_PACKAGE="$SANDBOX/outside-server"; cp -a "$OVERLAY_PREFIX" "$ESCAPE_PREFIX"; cp -a "$ESCAPE_PREFIX/lib/node_modules/paperclipai/node_modules/@paperclipai/server" "$ESCAPE_PACKAGE"; rm -rf "$ESCAPE_PREFIX/lib/node_modules/paperclipai/node_modules/@paperclipai/server"; ln -s "$ESCAPE_PACKAGE" "$ESCAPE_PREFIX/lib/node_modules/paperclipai/node_modules/@paperclipai/server"
capture escape_root_out escape_root_code node "$ENGINE_DIR/overlay-contract.mjs" "$ESCAPE_PREFIX" "$OVERLAY_BUILD" 0123456789012345678901234567890123456789 "$ESCAPE_PREFIX/.paperclip-engine-overlay.json"
assert_eq "package root escaping prefix fails closed" "1" "$escape_root_code"
