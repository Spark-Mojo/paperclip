...
describeEmbeddedPostgres("shared-workspace run serialization", () => {
  ...
  // SPA-5925 mirror of upstream #11293 — the cancel-write and the
  // scheduled-retry insert are two separate writes, so a direct read after
  // waitForRunToLeaveActiveStates can race ahead of the retry insert and
  // return null. Poll for the retry row instead of asserting immediately.
